# Aquí se piden los datos del problema y se elige el método de resolución.
try:
    from .numeros import texto_a_numero, EPS
...
def leer_problema():
    ...
    for i in range(m):
        print(f"\nRestricción {i+1}:")
        coefs = [pedir_numero(f"  a{i+1}{j+1}  (coeficiente de x{j+1}): ") for j in range(n)]
        signo = pedir_signo("  signo (<=, >=, =): ")
        b = pedir_numero("  b (lado derecho): ")
...
def metodos_disponibles(n, restricciones):
    disponibles = []
    # El simplex estándar solo acepta restricciones <= con b >= 0.
    solo_le = all(r[-2] == '<=' for r in restricciones)
    b_no_negativo = all(r[-1] >= -EPS for r in restricciones)

    if n == 2:
        disponibles.append('grafico')
    if solo_le and b_no_negativo:
        disponibles.append('simplex')
    disponibles.append('granm')  # la Gran M casi siempre está disponible
